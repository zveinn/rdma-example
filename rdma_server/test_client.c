...
static int client_prepare_connection(struct sockaddr_in *s_addr) {
  ...
  if (!cm_event_channel) {
    debug("Creating cm event channel failed, errno: %d \n", -errno);
    return -errno;
  }
  ...
  if (ret) {
    debug("Creating cm id failed with errno: %d \n", -errno);
    return -errno;
  }
  ...
  if (ret) {
    debug("Failed to resolve route, erno: %d \n", -errno);
    return -errno;
  }
  ...
  if (!pd) {
    debug("Failed to alloc pd, errno: %d \n", -errno);
    return -errno;
  }
  ...
  if (!io_completion_channel) {
    debug("Failed to create IO completion event channel, errno: %d\n", -errno);
    return -errno;
  }
  ...
  if (!client_cq) {
    debug("Failed to create CQ, errno: %d \n", -errno);
    return -errno;
  }
  debug("CQ created at %p with %d elements \n", client_cq, client_cq->cqe);
  ret = ibv_req_notify_cq(client_cq, 0);
  if (ret) {
    debug("Failed to request notifications, errno: %d\n", -errno);
    return -errno;
  }
  ...
  if (ret) {
    debug("Failed to create QP, errno: %d \n", -errno);
    return -errno;
  }
...
static int client_pre_post_recv_buffer() {
  ...
  if (!server_metadata_mr) {
    debug("Failed to setup the server metadata mr , -ENOMEM\n");
    return -ENOMEM;
  }
  ...
  if (ret) {
    debug("Failed to pre-post the receive buffer, errno: %d \n", ret);
    return ret;
  }
...
static int client_connect_to_server() {
  ...
  ret = process_rdma_cm_event(cm_event_channel, RDMA_CM_EVENT_ESTABLISHED,
                              &cm_event);
  if (ret) {
    debug("Failed to get cm event, ret = %d \n", ret);
    return ret;
  }
  ret = rdma_ack_cm_event(cm_event);
  if (ret) {
    debug("Failed to acknowledge cm event, errno: %d\n", -errno);
    return -errno;
  }
...
static int client_xchange_metadata_with_server() {
  ...
  if (!client_src_mr) {
    debug("Failed to register the first buffer, ret = %d \n", ret);
    return ret;
  }
  printf("D1 \n");
  ...
  if (!client_metadata_mr) {
    debug("Failed to register the client metadata buffer, ret = %d \n", ret);
    return ret;
  }
  printf("D2 \n");
  ...
  printf("D4 \n");
  printf("1:%p", client_qp);
  printf("2:%p", &client_send_wr);
  printf("3:%p", &bad_client_send_wr);
  printf("D4 \n");
  /* Now we post it */
  ret = ibv_post_send(client_qp, &client_send_wr, &bad_client_send_wr);
  if (ret) {
    debug("Failed to send client metadata, errno: %d \n", -errno);
    return -errno;
  }
  printf("D5 \n");
...
static int writeSource(char *data) {
  ...
  ret = ibv_post_send(client_qp, &client_send_wr, &bad_client_send_wr);
  if (ret) {
    debug("Failed to write client src buffer, errno: %d \n", -errno);
    return -errno;
  }
  /* at this point we are expecting 1 work completion for the write */
  ret = process_work_completion_events(io_completion_channel, &wc);
  if (ret != 1) {
    debug("We failed to get 1 work completions , ret = %d \n", ret);
    return ret;
  }
...
static int client_remote_memory_ops() {
  ...
  if (!client_dst_mr) {
    debug("We failed to create the destination buffer, -ENOMEM\n");
    return -ENOMEM;
  }
  ...
  ret = ibv_post_send(client_qp, &client_send_wr, &bad_client_send_wr);
  if (ret) {
    debug("Failed to read client dst buffer from the master, errno: %d\n ",
          errno);
    return -errno;
  }
...
static int client_disconnect_and_clean() {
  ...
  if (ret) {
    debug("Failed to disconnect, errno: %d \n", -errno);
    // continuing anyways
  }
  ret = process_rdma_cm_event(cm_event_channel, RDMA_CM_EVENT_DISCONNECTED,
                              &cm_event);
  if (ret) {
    debug("Failed to get RDMA_CM_EVENT_DISCONNECTED event, ret = %d\n", ret);
    // continuing anyways
  }
  ret = rdma_ack_cm_event(cm_event);
  if (ret) {
...
int main(int argc, char **argv) {
  ...
  while ((option = getopt(argc, argv, "s:a:p:")) != -1) {
    ...
    case 's':
      ...
      if (!src) {
        debug("Failed to allocate memory : -ENOMEM\n");
        return -ENOMEM;
      }
      ...
      if (!dst) {
        debug("Failed to allocate destination memory, -ENOMEM\n");
        free(src);
        return -ENOMEM;
      }
      break;
  ...
  if (ret) {
    debug("Failed to setup client connection , ret = %d \n", ret);
    return ret;
  }
  ...
  if (ret) {
    debug("Failed to finish remote memory ops, ret = %d \n", ret);
    return ret;
  }
  // if (check_src_dst()) {
  //   debug("src and dst buffers do not match \n");
  // } else {
  ...
  // if (ret) {
  //   debug("Failed to cleanly disconnect and clean up resources \n");
  // }
  //
  while (1) {
    ret = writeSource("11");
